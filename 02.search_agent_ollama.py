...
class AgentResponse(BaseModel):
    ...
    sources: List[Source] = Field(
        ...
        description="The sources used by the agent to answer the question",
    )
    ...
    result = agent.invoke(
        {
            "messages": [
                HumanMessage(
                    content="Search for 3 job postings for ai engineer on linkedin from us or europe that allow international hires from brazil"
                )
            ]
        }
    )
